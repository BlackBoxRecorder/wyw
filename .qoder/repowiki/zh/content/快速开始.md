...
## 更新摘要
**变更内容**
- 更新版本信息：从 0.1.4 升级到 0.1.5
- 项目名称变更：包名为 @bbr/wyw，可执行命令为 wenyanwen
- 增强文档内容：README.md 中添加了更多功能特性和使用说明
- 命令行工具更新：支持新的命令名称和增强的 CLI 功能
...
**更新** 版本现已升级至 0.1.5，项目名称变更为 @bbr/wyw，提供更稳定的编译体验和增强的功能特性。
...
**更新** 新版本增强了 CLI 功能，支持更灵活的命令行选项和更好的错误处理机制。
...
U->>CLI : "wenyanwen build examples/岳阳楼记.wyw"
...
- 安装步骤
  - 全局安装（推荐）：npm install -g @bbr/wyw
  - 本地安装：npm install @bbr/wyw
  - 构建产物：npm run build
- 验证安装
  - 查看版本：wenyanwen --version
  - 查看帮助：wenyanwen --help
  - 使用 npx（无需全局安装）：npx @bbr/wyw --help

**更新** 项目名称已变更为 @bbr/wyw，命令行工具名称变更为 wenyanwen，提供更清晰的包标识和命令区分。
...
### 基本使用流程
- 创建第一个 .wyw 文件
  - 使用 init 命令生成模板：wenyanwen init
  - 或参考 examples/范仲淹_岳阳楼记.wyw 的结构与语法
- 编译为 HTML
  - 单文件编译：wenyanwen build examples/岳阳楼记.wyw
  - 指定输出目录：wenyanwen build examples/*.wyw -o examples/dist/
  - 监听模式：wenyanwen build examples/岳阳楼记.wyw -w
  - 内联资源：wenyanwen build examples/岳阳楼记.wyw --inline
- 预览与分享
  - 在浏览器中打开生成的 HTML 文件
  - 可根据需要调整主题与字号

**更新** 命令行工具已更新为 wenyanwen，提供更直观的文言文编译体验。
...
**更新** 新版本增强了命令行选项的灵活性，提供更好的用户体验和更丰富的配置选择。
...
**更新** 语法文档已增强，提供更多实际示例和详细的语法说明，帮助用户更好地理解和使用文言文标记语言。
...
**更新** 新版本优化了性能表现，提供更流畅的编译体验和更好的资源管理。
...
## 故障排除指南
- 安装相关
  - 若全局安装失败，请尝试使用 npm install -g @bbr/wyw 或检查权限
  - 若 npx 无法找到 wenyanwen，请确认 PATH 或使用本地安装
- 命令行错误
  - 使用 --help 查看可用命令与选项
...
**更新** 新版本增强了错误处理和诊断功能，提供更清晰的错误信息和解决方案。
...
**更新** 版本 0.1.5 提供了更稳定和强大的功能，包括改进的命令行工具、增强的语法支持和更好的用户体验。

## 附录
- 常用命令速查
  - 初始化模板：wenyanwen init
  - 编译单文件：wenyanwen build examples/岳阳楼记.wyw
  - 批量编译：wenyanwen build examples/*.wyw -o examples/dist/
  - 监听模式：wenyanwen build examples/岳阳楼记.wyw -w
  - 校验文件：wenyanwen validate examples/岳阳楼记.wyw
  - 校验文件（严格模式）：wenyanwen validate examples/岳阳楼记.wyw --strict
