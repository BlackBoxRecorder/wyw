...
- [src/validator.ts: 1-804:1-804](file://src/validator.ts#L1-L804)
- [src/cli.ts](file://src/cli.ts)
...
## 结论
本语法参考系统梳理了 .wyw 的文件结构、块级与内联语法、注音与注释、译文与围栏块，并结合解析器与验证器的实现细节，提供了优先级规则、嵌套限制与常见错误的解决方案。建议在写作时遵循示例与最佳实践，使用验证器保障格式质量，借助 CLI 工具高效生成 HTML。
